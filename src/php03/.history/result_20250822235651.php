...
// isset()を使う理由ースーパーグローバル変数（$_POST, $_GET, $_SESSION など）に存在しないキーを直接参照しようとすると、「未定義の変数（Undefined index）」という警告が出ます。isset() を使うことで、「そのキーが存在しているかどうか」を確認してから使うことができます。存在しない場合は処理をスキップできるので、安全で安定したコードになります。
